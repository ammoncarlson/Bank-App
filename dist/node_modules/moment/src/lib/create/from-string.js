...
function configFromISO(config) {
    ...
    if (match) {
        parsing_flags_1.default(config).iso = true;
...
function extractFromRFC2822Strings(yearStr, monthStr, dayStr, hourStr, minuteStr, secondStr) {
    var result = [
        ...
        month_1.defaultLocaleMonthsShort.indexOf(monthStr),
        parseInt(dayStr, 10),
        parseInt(hourStr, 10),
...
function checkWeekday(weekdayStr, parsedInput, config) {
    if (weekdayStr) {
        ...
        var weekdayProvided = day_of_week_1.defaultLocaleWeekdaysShort.indexOf(weekdayStr), weekdayActual = new Date(parsedInput[0], parsedInput[1], parsedInput[2]).getDay();
...
function configFromRFC2822(config) {
    ...
    if (match) {
        ...
        config._d = date_from_array_1.createUTCDate.apply(null, config._a);
        config._d.setUTCMinutes(config._d.getUTCMinutes() - config._tzm);
        parsing_flags_1.default(config).rfc2822 = true;
    }
    else {
        config._isValid = false;
    }
...
hooks_1.hooks.createFromInputFallback = deprecate_1.deprecate('value provided is not in a recognized RFC2822 or ISO format. moment construction falls back to js Date(), ' +
